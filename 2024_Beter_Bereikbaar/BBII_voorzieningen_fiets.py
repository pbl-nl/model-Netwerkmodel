...
# type car network
type_carnetwork = "OSM2022" #tomtom zonder jaar invullen (staat verderop bij datum), indien geprojecteerd op osm jaar osm invullen
...
tijdblok = "" #MonTue #WedThuFri #SatSun
if tijdblok != "":
...
# Naam run
indicator = "VO2022_"
koppel_identificatie = "VO2022_ID"
if type_carnetwork <> "":
...
# Instellen temporele locatie tbv hernoemen bestanden
temp_workspace_output = os.path.join("C:\RN", run+"_"+wachttijd+"_"+walkbike+"_"+gem_hour+subset)
settings_dict["temp_workspace_output"] = temp_workspace_output
settings_dict_uitleg["temp_workspace_output"] = "Temporele locatie tbv hernoemen bestanden"
...
geodms_ids_file = os.path.join(geodms_output_dir, "Ziekenhuizen_apr2022_alg_ac_xy_GEODMS.csv")
...
#         full_filename_new = full_filename_old.replace("Traveltime", "TT").replace("Departure", "Dep").replace("centroiden", "centr").replace("ziekenhuizen", "zh").replace("Buurt", "Brt").replace("00s", "").replace("_y2020", "2020").replace("Long", "Lng").replace("DEST", "DST").replace("with", "w").replace("Dep_At", "Dep").replace("Bic_OV_W_w_W_OV_W_BB", "BicOVWwWOVWBB").replace("TT_Lng_", "TTLng").replace("W_OV_W_w_WW", "WOVWwWW").replace("huisartsenpraktijken", "haprak").replace("O2SWtime-", "").replace("basisscholen", "bschool").replace("voortgezet_onderwijs", "vo")
...
# controle of alle buurtnummers voorkomen in de tabel
data['OrgName'].nunique()
...
#         arcpy.CopyFeatures_management(polygons, fgdb_polygon_out_file)
    ...
    inw_2022 = pd.read_csv(r"d:\RN\brt2023.txt", sep=";")
